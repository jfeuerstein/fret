...
export function WfOrb({
  ...
  const proj = verts3d.map(([x, y]) => [x, y]);
  const r = size * 0.35;
  return (
    <svg
      width={size}
...
export function WfCassette({
  ...
  return (
    ...
    >
      <path
        d="M 12 20 L 168 20 L 172 30 L 172 95 L 168 100 L 12 100 L 8 95 L 8 30 Z"
        fill="none" stroke={color} strokeWidth="1.2"
      />
      <path
        d="M 22 30 L 158 30 L 158 55 L 22 55 Z"
...
export function WfHeadstock({
  ...
  return (
    <svg width={width} height={height} viewBox="0 0 80 140" style={{ display: "block" }}>
      ...
      <path
        d="M 30 110 L 24 100 L 18 92 L 14 80 L 12 65 L 14 48 L 20 32 L 30 20 L 44 12 L 56 14 L 60 22 L 58 30 L 50 36 L 44 44 L 42 58 L 44 72 L 48 88 L 50 100 L 50 110 Z"
        fill="none" stroke={color} strokeWidth="1.2"
      />
      {[28, 48, 68, 88].map((y) => (
        <g key={y}>
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: "block" }}>
      {frets.map((f, i) => {
        ...
        if (f === -1)
          return (
            <text key={i} x={x} y={y} fontSize={w * 0.13} fill={muted}
              textAnchor="middle" fontFamily="Courier New, monospace">x</text>
          );
        if (f === 0)
          return (
            <circle key={i} cx={x} cy={y} r={w * 0.05} fill="none" stroke={color} strokeWidth="1" />
          );
        return null;
      })}
      {startFret === 1 && (
        <line x1={padX - 2} y1={padTop} x2={padX + usableW + 2} y2={padTop}
          stroke={color} strokeWidth="2" />
      )}
      {startFret > 1 && (
        <text x={padX - 6} y={padTop + fretSpacing * 0.7} fontSize={w * 0.14} fill={muted}
          textAnchor="end" fontFamily="Courier New, monospace">{startFret}fr</text>
      )}
      {Array.from({ length: visibleFrets + 1 }).map((_, i) => (
        <line key={`f${i}`} x1={padX} y1={padTop + i * fretSpacing}
          x2={padX + usableW} y2={padTop + i * fretSpacing}
          stroke={color}
          strokeWidth={i === 0 && startFret === 1 ? 0 : 0.8}
          opacity="0.6" />
      ))}
      {Array.from({ length: stringCount }).map((_, i) => (
        <line key={`s${i}`} x1={padX + i * stringSpacing} y1={padTop}
        ...
        return (
          <g key={`d${i}`}>
            ...
            {chord.fingers && chord.fingers[i] > 0 && (
              <text x={x} y={y + w * 0.03} fontSize={w * 0.11} fill="#0a0a0a"
                textAnchor="middle" fontFamily="Courier New, monospace" fontWeight="bold">
                {chord.fingers[i]}
              </text>
            )}
          </g>
      ...
      <text x={w / 2} y={h - 2} fontSize={w * 0.16} fill={color}
