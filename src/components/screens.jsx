...
const sc = {
  bg: '#0a0a0a', panel: '#111', fg: '#e0e0e0', dim: '#999', muted: '#666',
  faint: '#3a3a3a', border: '#333', green: '#44ff44', amber: '#ff8844', red: '#ff6b6b',
};
...
function Onboarding({ onDone }) {
  ...
  const steps = [
    {
      ...
      art: <window.WfHeadstock width={120} height={180} color={sc.dim}/>,
      body: (
        <div style={{ color: sc.dim, fontSize: 13, lineHeight: 1.6 }}>
          fret builds you a daily practice, gym-style. each day's a tape, each block a track. takes 30 seconds to set up.
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      body: (
        <Choices
          ...
          onChange={v => setProfile(p => ({ ...p, focuses: v.length ? v : ['technique'] }))}
        />
      ),
    },
    {
      ...
      art: <window.WfOrb size={120} color={sc.amber} accent={sc.amber}/>,
      body: (
        <div style={{ color: sc.dim, fontSize: 13, lineHeight: 1.6 }}>
          {profile.daysPerWeek} days/wk · {profile.sessionLength}m sessions · cycling: {profile.focuses.join(' / ')}
        </div>
      ),
    },
  ];
  ...
  return (
    <div style={{ padding: '74px 22px 100px', minHeight: '100%', boxSizing: 'border-box', background: sc.bg, color: sc.fg, fontFamily: sFont, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ fontSize: 24, marginBottom: 4, textTransform: 'lowercase' }}>{cur.title}</div>
      <div style={{ color: sc.dim, fontSize: 13, marginBottom: 22 }}>{cur.sub}</div>

      {cur.art && <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 24 }}>{cur.art}</div>}
...
const obtnGhost = {
  flex: 1, background: 'transparent', color: sc.fg,
  border: `1px solid ${sc.border}`, padding: '14px 0',
  font: 'inherit', fontFamily: sFont, fontSize: 13,
  letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer',
};
const obtnSolid = {
  ...obtnGhost, flex: 2, background: sc.fg, color: sc.bg, borderColor: sc.fg,
};
...
function Slider({ min, max, step, value, onChange, unit }) {
  return (
    <div style={{
      border: `1px solid ${sc.border}`, padding: '20px 16px',
      textAlign: 'center', background: sc.panel,
    }}>
      <div style={{ fontSize: 56, lineHeight: 1, color: sc.amber, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
      <div style={{ color: sc.muted, fontSize: 11, letterSpacing: 2, marginTop: 4, textTransform: 'uppercase' }}>{unit}</div>
      <input
...
function JournalScreen() {
  ...
  return (
    <div style={{ padding: '74px 16px 100px', minHeight: '100%', boxSizing: 'border-box', background: sc.bg, color: sc.fg, fontFamily: sFont, fontSize: 13 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', color: sc.muted, fontSize: 10, letterSpacing: 3, marginBottom: 14 }}>
        <span>JOURNAL</span><span>{entries.length} ENTRIES</span>
      </div>
      ...
      <div style={{ border: `1px solid ${sc.border}`, padding: 12, marginBottom: 14, background: sc.panel }}>
        <div style={{ color: sc.muted, fontSize: 10, letterSpacing: 2, marginBottom: 8 }}>NEW ENTRY</div>
        <textarea
          value={text}
          ...
          style={{
            ...
            background: 'transparent', color: sc.fg,
            border: `1px solid ${sc.border}`, borderRadius: 0,
            padding: 10, font: 'inherit', fontFamily: sFont,
        ...
        <div style={{ display: 'flex', gap: 4, marginTop: 8, marginBottom: 8 }}>
          {['rough', 'meh', 'solid', 'fire'].map(m => (
            <button key={m} onClick={() => setMood(m)} style={{
              ...
              padding: '8px 0', font: 'inherit', fontFamily: sFont,
              fontSize: 11, letterSpacing: 1, cursor: 'pointer',
            }}>{m}</button>
          ))}
        </div>
        <button onClick={submit} disabled={!text.trim()} style={{
          width: '100%', background: text.trim() ? sc.fg : sc.faint,
          color: sc.bg, border: 'none', padding: '12px 0',
          font: 'inherit', fontFamily: sFont, fontSize: 12,
          letterSpacing: 2, cursor: text.trim() ? 'pointer' : 'not-allowed',
        }}>+ SAVE ENTRY</button>
      </div>

      <div style={{ color: sc.muted, fontSize: 10, letterSpacing: 2, marginBottom: 8 }}>HISTORY</div>
      {entries.length === 0 ? (
        ...
        <div key={i} style={{
          padding: '12px 14px', border: `1px solid ${sc.border}`,
          marginBottom: 6, background: sc.panel, fontSize: 12,
        }}>
