...
// screens: today (home), session, week, journal, tuner, library
...
import { drone, metronome, tuner } from "../audio.js";
import { generateSession } from "../generator.js";
import { JournalScreen } from "./screens.jsx";
import {
  ChordDiagram,
  WfCassette,
  WfOrb,
...
function Btn({
  ...
  const pad =
    size === "lg" ? "16px 18px" : size === "sm" ? "8px 10px" : "12px 14px";
  return (
    <button
      onClick={onClick}
...
function Section({ title, right, children, mt = 18 }) {
  return (
    <div style={{ marginTop: mt }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", color: c.muted, fontSize: 10, letterSpacing: 2, marginBottom: 8 }}>
        <span>{title}</span>
...
function HomeScreen({ goSession, goLibrary }) {
  ...
  useEffect(() => {
    ...
    const focus = todayPlan && todayPlan.focus !== "rest" ? todayPlan.focus : "technique";
    setT(generateSession({ focus, sessionLength: profile.sessionLength, history }));
  }, [t, profile, history, week]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 4px 0", borderTop: `1px dashed ${c.border}`, marginTop: 4 }}>
      ...
      <Btn size="sm" onClick={() => setT(null)}>↺ REROLL TAPE</Btn>
...
// ─── session ──────────────────────────────────────────────────
function SessionScreen({ onComplete }) {
  const profile = useStore((s) => s.profile);
  const history = useStore((s) => s.history);
  const week = useStore((s) => s.week);
  const [t, setT] = useState(null);
  ...
  // build session once profile is ready
  useEffect(() => {
    if (!profile) return;
    if (t) return;
    const today = todayDayKey();
    const todayPlan = (week || []).find((d) => d.day === today);
    const focus = todayPlan && todayPlan.focus !== "rest" ? todayPlan.focus : "technique";
    const session = generateSession({ focus, sessionLength: profile.sessionLength, history });
    setT(session);
    if (session?.blocks?.[0]?.bpm) setBpm(session.blocks[0].bpm);
  }, [profile, history, week, t]);
  ...
  const next = () => {
    ...
    if (activeIdx < t.blocks.length - 1) {
      ...
      setPlaying(false);
      Store.completeSession(t, t.duration);
      onComplete?.();
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* big bpm */}
      {block.bpm > 0 ? (
        <div style={{ border: `1px solid ${c.border}`, padding: "14px 16px 16px", marginBottom: 14, textAlign: "center", position: "relative" }}>
...
function WeeklyScreen() {
  ...
  const todayIdx = Math.max(0, w.findIndex((d) => d.day === todayKey));
  ...
  return (
    ...
    >
      ...
      <div style={{ border: `1.5px solid ${c.fg}`, padding: "14px 16px", marginBottom: 14, background: c.panel }}>
        ...
        {sel.status !== "rest" && (
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 14, borderTop: `1px dashed ${c.border}`, paddingTop: 10 }}>
...
function Mini({ label, value, accent }) {
  return (
    <div style={{ textAlign: "center", flex: 1 }}>
      ...
      <div style={{ fontSize: 16, color: accent ? c.amber : c.fg, fontVariantNumeric: "tabular-nums" }}>
        {value}
      </div>
    </div>
  );
...
function TunerScreen() {
  ...
  return (
    ...
    >
      ...
      {permission === "granted" && (
        <>
          <div style={{ border: `1px solid ${c.border}`, padding: "24px 16px", textAlign: "center", marginBottom: 16, position: "relative" }}>
...
export function TapeApp() {
  ...
  const screens = {
    ...
    session: <SessionScreen onComplete={() => setTab("home")} />,
    weekly: <WeeklyScreen />,
