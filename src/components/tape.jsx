...
// ─── shared atoms ─────────────────────────────────────────────
function Btn({ children, onClick, variant = 'ghost', full = true, size = 'md', style }) {
...
function HomeScreen({ goSession, goLibrary }) {
  const t = window.TODAY, s = window.STATS;
  const [hover, setHover] = useState(false);

  return (
    <div style={{ padding: '74px 16px 100px', minHeight: '100%', boxSizing: 'border-box', background: c.bg, color: c.fg, fontFamily: baseFont, fontSize: 13 }}>
      <Header left="SAT · MAY 09" right={`DAY ${String(t.day).padStart(3, '0')}`}/>
      ...
      >
        <div style={{
          border: `1px solid ${c.border}`, padding: '10px 12px', marginBottom: 14, background: c.bg,
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2 }}>SIDE A</div>
            <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2 }}>{t.duration} MIN · {t.blocks.length} TRK</div>
          </div>
          <div style={{ fontSize: 22, marginTop: 6, textTransform: 'uppercase', letterSpacing: 1 }}>{t.title}</div>
          <div style={{ color: c.dim, fontSize: 12, marginTop: 2 }}>{t.subtitle}</div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 18, padding: '6px 0 10px' }}>
          <window.WfCassette width={240} height={140} color="#5a5a5a" accent={hover ? c.amber : '#888'}/>
        </div>

        <div style={{
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          padding: '8px 4px 0', borderTop: `1px dashed ${c.border}`, marginTop: 4,
        }}>
          <div style={{ color: c.amber, fontSize: 10, letterSpacing: 2 }}>★ STREAK {String(s.streak).padStart(3, '0')}</div>
          <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2 }}>{s.totalAllTime} ALL-TIME</div>
        </div>
      </div>

      {/* big play */}
      <button onClick={goSession} style={{
        width: '100%', background: c.fg, color: c.bg, border: 'none',
        padding: '20px 0', font: 'inherit', fontFamily: baseFont, fontSize: 18,
        letterSpacing: 4, cursor: 'pointer', marginBottom: 8,
      }}>▷ PLAY SIDE A</button>
      ...
      <Section title="TRACKLIST" right="BPM · TIME" mt={20}>
        {t.blocks.map((b, i) => (
          <div key={b.id} style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            borderBottom: i === t.blocks.length - 1 ? 'none' : `1px dashed ${c.border}`,
            padding: '8px 0', fontSize: 13,
          }}>
            <div style={{ display: 'flex', gap: 10, alignItems: 'baseline' }}>
              <span style={{ color: c.muted, fontSize: 11, width: 22 }}>A{String(i + 1).padStart(2, '0')}</span>
              <span>{b.label}</span>
            </div>
      ...
      <Section title="LAST PLAY">
        <div style={{
          padding: '12px 14px', border: `1px solid ${c.border}`, background: c.panel,
        }}>
          <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2 }}>{s.lastSession.day.toUpperCase()}</div>
          <div style={{ color: c.dim, fontSize: 12, marginTop: 6, lineHeight: 1.5, fontStyle: 'italic' }}>
            "{s.lastSession.note}"
          </div>
        </div>
      </Section>
...
function SessionScreen() {
  ...
  const [done, setDone] = useState({ warmup: true, dexterity: true });
    ...
    <div style={{ padding: '74px 16px 100px', minHeight: '100%', boxSizing: 'border-box', background: c.bg, color: c.fg, fontFamily: baseFont, fontSize: 13 }}>
      ...
      <div style={{ marginBottom: 14, borderBottom: `1px solid ${c.border}`, paddingBottom: 12 }}>
        <div style={{ color: c.muted, fontSize: 11, letterSpacing: 2, marginBottom: 4 }}>
          TRACK {String(activeIdx + 1).padStart(2, '0')} / {String(t.blocks.length).padStart(2, '0')}
          {' · '}
      ...
      {block.bpm > 0 ? (
        <div style={{
          border: `1px solid ${c.border}`, padding: '14px 16px 16px',
        ...
        <div style={{
          border: `1px dashed ${c.border}`, padding: '20px 16px',
          marginBottom: 14, textAlign: 'center', color: c.muted, fontSize: 12,
        }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: c.muted, marginBottom: 4, fontVariantNumeric: 'tabular-nums' }}>
            ...
            <div style={{ height: 4, background: c.faint, position: 'relative' }}>
              <div style={{
                ...
                width: `${pct * 100}%`, background: c.fg,
              }}/>
            </div>
          </div>
          <button onClick={() => setPlaying(p => !p)} style={{
            ...knobBtn, width: 44, padding: '8px 0', fontSize: 16, flex: 'none',
            background: playing ? 'transparent' : c.fg, color: playing ? c.fg : c.bg,
          }}>
            {playing ? '⏸' : '▷'}
          </button>
        </div>
      ...
      <div style={{
        ...
        padding: '12px 14px', border: `1px solid ${c.border}`, marginBottom: 14,
      }}>
      ...
      {tabData && (
        <div style={{
          border: `1px solid ${c.border}`, padding: '10px 12px', marginBottom: 14,
          background: c.panel2,
        }}>
          <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2, marginBottom: 6 }}>TAB · {tabData.name.toUpperCase()}</div>
          <pre style={{
            margin: 0, fontFamily: baseFont, fontSize: 11, color: c.fg,
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
        <button onClick={prev} style={{ ...transport, flex: 1 }}>⏮</button>
        <button onClick={next} style={{ ...transport, flex: 1 }}>⏭</button>
      </div>
      <button onClick={next} style={{
        width: '100%', background: c.green, color: c.bg, border: 'none',
        padding: '14px 0', font: 'inherit', fontFamily: baseFont,
        ...
        {t.blocks.map((b, i) => {
          ...
          return (
            <div key={b.id} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '6px 0',
              color: isActive ? c.amber : (isDone ? c.muted : c.dim),
              fontSize: 12, opacity: isDone ? 0.6 : 1,
            }}>
              <div style={{ display: 'flex', gap: 10 }}>
                <span style={{ color: c.muted, width: 22 }}>A{String(i + 1).padStart(2, '0')}</span>
                <span style={{ textDecoration: isDone ? 'line-through' : 'none' }}>{b.label}</span>
              </div>
...
const knobBtn = {
  ...
  padding: '8px 0', font: 'inherit', fontFamily: baseFont,
...
function WeeklyScreen() {
  const w = window.WEEK, s = window.STATS;
  ...
  return (
    <div style={{ padding: '74px 16px 100px', minHeight: '100%', boxSizing: 'border-box', background: c.bg, color: c.fg, fontFamily: baseFont, fontSize: 13 }}>
      <div style={{ marginBottom: 6 }}>
        ...
        <div style={{ fontSize: 22, marginTop: 4 }}>w. of may 04</div>
      </div>
      <div style={{ display: 'flex', gap: 10, color: c.muted, fontSize: 11, marginBottom: 18 }}>
        ...
        <span style={{ color: c.faint }}>·</span>
        <span style={{ color: c.amber }}>★ {s.streak}d</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 5, marginBottom: 18 }}>
        {w.map((d, i) => {
          ...
          return (
            <div key={d.day} onClick={() => setSelected(i)}
              style={{
                flex: 1, cursor: 'pointer',
                border: `1.5px solid ${isSel ? c.fg : (isToday ? c.amber : c.border)}`,
                background: isSel ? c.panel : 'transparent',
                padding: '10px 4px 6px', textAlign: 'center', position: 'relative',
                opacity: isRest ? 0.45 : 1, transition: 'all 0.15s',
              }}
            >
              <div style={{ color: isToday ? c.amber : c.muted, fontSize: 9, letterSpacing: 1.5 }}>{d.day.toUpperCase()}</div>
              <div style={{ height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '4px 0' }}>
                {isRest ? <span style={{ color: c.muted, fontSize: 16 }}>—</span> : <Reels size={18} spinning={isToday}/>}
              </div>
              <div style={{ fontSize: 10, color: isDone ? c.green : (isToday ? c.amber : c.muted) }}>
      ...
      {/* selected detail */}
      <div style={{
        border: `1.5px solid ${c.fg}`, padding: '14px 16px',
        marginBottom: 14, background: c.panel,
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
          <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2 }}>{sel.day.toUpperCase()} · {sel.date.split(' ')[1]}</div>
          <div style={{
            color: sel.status === 'done' ? c.green : sel.status === 'today' ? c.amber : c.muted,
            fontSize: 10, letterSpacing: 2,
          }}>
            {sel.status === 'done' ? '✓ PLAYED' : sel.status === 'today' ? '▷ ON DECK' : sel.status === 'rest' ? '— REST' : 'QUEUED'}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 14 }}>
        <Mini label="STREAK" value={`${s.streak}d`} accent/>
      ...
      <Section title="JOURNAL" mt={20}>
        {w.filter(d => d.status === 'done').slice(-3).map((d, i) => (
          <div key={i} style={{
            padding: '10px 12px', borderBottom: `1px dashed ${c.border}`,
            fontSize: 12, color: c.dim,
          }}>
            <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2, marginBottom: 4 }}>
              {d.day.toUpperCase()} · {d.focus}
            </div>
            <div style={{ fontStyle: 'italic' }}>"got box 1 clean at {d.bpm}bpm — felt the rhythm lock in."</div>
          </div>
        ))}
      </Section>
    </div>
...
function Mini({ label, value, accent }) {
  return (
    <div style={{ textAlign: 'center', flex: 1 }}>
      <div style={{ color: c.muted, fontSize: 9, letterSpacing: 1.5, marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 16, color: accent ? c.amber : c.fg, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
    </div>
  );
}
...
function TunerScreen() {
  ...
  return (
    <div style={{ padding: '74px 16px 100px', minHeight: '100%', boxSizing: 'border-box', background: c.bg, color: c.fg, fontFamily: baseFont, fontSize: 13 }}>
      ...
      {permission === 'idle' && (
        <div style={{
          border: `1px solid ${c.border}`, padding: '24px 16px',
          textAlign: 'center', marginBottom: 16,
        }}>
          <div style={{ marginBottom: 14, display: 'flex', justifyContent: 'center' }}>
            <window.WfOrb size={70} color={c.muted} accent={c.amber}/>
          </div>
          <div style={{ color: c.dim, fontSize: 12, marginBottom: 14, lineHeight: 1.5 }}>
            this'll need your mic for a sec.<br/>
      ...
      {permission === 'denied' && (
        <div style={{
          border: `1px solid ${c.red}`, padding: '14px 16px',
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              color: note ? (Math.abs(note.cents) < 5 ? c.green : Math.abs(note.cents) < 15 ? c.amber : c.fg) : c.faint,
            ...
            <div style={{ marginTop: 18, position: 'relative' }}>
              <div style={{
                height: 4, background: c.faint, position: 'relative',
              }}>
                {/* center line */}
                <div style={{
                  position: 'absolute', left: '50%', top: -4, bottom: -4,
                  width: 1, background: c.fg,
                }}/>
                {/* needle */}
                {note && (
                  <div style={{
                    position: 'absolute', top: -6, height: 16, width: 3,
            ...
            {note && (
              <div style={{
                marginTop: 12, color: Math.abs(note.cents) < 5 ? c.green : c.muted,
                fontSize: 11, letterSpacing: 2,
              }}>
                {Math.abs(note.cents) < 5 ? '✓ in tune' :
                 note.cents > 0 ? `↓ flatten ${note.cents}¢` : `↑ tighten ${-note.cents}¢`}
              </div>
            )}
          </div>
          ...
          {/* live waveform */}
          <div style={{
            border: `1px solid ${c.border}`, padding: '10px 12px', marginBottom: 16,
            background: c.panel,
          }}>
            <div style={{ color: c.muted, fontSize: 10, letterSpacing: 2, marginBottom: 6 }}>SIGNAL</div>
            <window.WfWave width={320} height={36} color={c.amber} active/>
          </div>
          ...
          <div style={{ marginTop: 16 }}>
            <Btn size="sm" onClick={() => { tuner.stop(); setPermission('idle'); setPitch(null); setSmoothPitch(null); }}>
              ⏸ stop tuner
            </Btn>
          </div>
        </>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
        {['chords', 'tabs', 'backing'].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            flex: 1, background: tab === t ? c.fg : 'transparent',
            color: tab === t ? c.bg : c.fg,
            border: `1px solid ${tab === t ? c.fg : c.border}`,
            padding: '10px 0', font: 'inherit', fontFamily: baseFont,
      ...
      <div style={{
        ...
        {chords.map(ch => (
          <div key={ch.name} style={{
            ...
            display: 'flex', justifyContent: 'center', background: c.panel,
          }}>
...
function TabsTab() {
  ...
  return (
    <>
      ...
      {tabs.map(([key, t]) => (
        <div key={key} style={{
          border: `1px solid ${c.border}`, padding: '12px 14px', marginBottom: 10,
          background: c.panel,
        }}>
          ...
          <pre style={{
            margin: 0, fontFamily: baseFont, fontSize: 11,
            color: c.fg, lineHeight: 1.5, overflowX: 'auto', whiteSpace: 'pre',
          }}>{t.lines.join('\n')}</pre>
        </div>
      ))}
    </>
      ...
      {tracks.map(([id, t]) => {
        ...
        return (
          <div key={id} style={{
            ...
            <div style={{ color: c.dim, fontSize: 12, marginBottom: 6 }}>{t.chords.join(' / ')}</div>
      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            fontFamily: baseFont, fontSize: 10, letterSpacing: 1.5,
            textTransform: 'uppercase',
...
function TapeApp() {
  ...
  const [storeState, setStoreState] = useState(Store ? Store.get() : null);
  ...
  // onboarding gate
  if (window.Onboarding && storeState && !storeState.profile) {
    return (
      <div style={{ width: '100%', height: '100%', position: 'relative', background: c.bg, overflow: 'auto' }}>
  ...
  const screens = {
    home: <HomeScreen goSession={() => setTab('session')} goLibrary={() => setTab('library')}/>,
    session: <SessionScreen/>,
    weekly: <WeeklyScreen/>,
    journal: window.JournalScreen ? <window.JournalScreen/> : <div/>,
    tuner: <TunerScreen/>,
