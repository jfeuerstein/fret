...
export const metronome = (() => {
  let bpm = 90, beatsPerBar = 4, nextNoteTime = 0, beat = 0, timer = null;
  let running = false, onBeat = null, volume = 0.4;
  ...
  function click(time, accent) {
    ...
    gain.gain.linearRampToValueAtTime(volume * (accent ? 1.0 : 0.7), time + 0.001);
  ...
  function schedule() {
    ...
    while (nextNoteTime < c.currentTime + 0.12) {
      ...
      const delay = Math.max(0, (fireAt - c.currentTime) * 1000);
      const b = beat;
...
// ─── tuner ────────────────────────────────────────────────────
export const tuner = (() => {
  let stream = null, analyser = null, raf = null, onPitch = null, buffer = null;
  ...
  return {
    ...
    stop() {
      ...
      stream = null; analyser = null; onPitch = null;
    },
...
export const drone = (() => {
  ...
  const chordIntervals = { maj:[0,4,7], min:[0,3,7], '5':[0,7], '7':[0,4,7,10], 'm7':[0,3,7,10] };
  ...
  function parseChord(name) {
    ...
    if (!f) return null;
    return { root, freq: f, intervals: chordIntervals[qual] || chordIntervals.maj };
  }

  let stopFn = null, running = false;
  return {
    start(track) {
      if (running) return;
      running = true;
      ...
      function playChord(name, startT, dur) {
        ...
        chord.intervals.forEach(semis => {
          const f = chord.freq * Math.pow(2, semis / 12) / 2;
          ...
          g.gain.linearRampToValueAtTime(peak, startT + 0.05);
          g.gain.linearRampToValueAtTime(peak * 0.7, startT + dur * 0.6);
      ...
      function scheduleNext() {
        ...
        while (t < c2.currentTime + 0.3) {
          playChord(track.chords[idx % track.chords.length], t, barLen);
          t += barLen; idx++;
        }
    ...
    stop() { if (stopFn) stopFn(); stopFn = null; running = false; },
    get running() { return running; },
  };
})();
